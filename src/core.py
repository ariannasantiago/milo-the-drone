...
class Core:
    ...
    def modeCallback(self, _, _1, _2) -> None:
        ...
        elif self.vehicle.mode.name == "GUIDIED":
            if self.isReady() and self.isConnected():
                # go into LOITER mode because this is totally undefined
                # pilot needs to set drone down and switch back into GUIDED
                # TODO: actually want to go back into core flow from guided mode tho..
    ...
    def isReady(self) -> bool:
        return self.vehicle.armed and \
                ...
                self.vehicle.location.global_relative_frame.alt > 0.5 #Indicates that we are actually flying
    
    def isAltitudeOk(self) -> bool:
        # TODO: Doesn't account for altitude at current position, only from home
    ...
    def run(self) -> None:
        while not self.state is ExecutionState.Stop:
            ...
            if self.state is ExecutionState.Init:
                ...
                # If state change happens during previous inner loop
                if self.state != ExecutionState.Takeoff: continue

                print('Take off to ' + str(ALTITUDE) + 'm')
                self.vehicle.simple_takeoff(ALTITUDE)

                # Wait until vehicle reaches altitude
